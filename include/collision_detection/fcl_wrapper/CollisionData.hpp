...
struct CollisionInformation
{
    ...
    /// @brief stores whether objects collided or not (use std::find_if, to get
    /// the first collision, if the stopping criterion is turned on)
    std::unordered_map<std::pair<std::string, std::string>, bool, LinkPairHash, LinkPairEqual>
        collisions;
...
struct DistanceData
{
    ...
    /// @brief Information regarding the collision distance (pre-allocated
    /// during initialization)
    std::unordered_map<std::pair<std::string, std::string>, DistanceInformation, LinkPairHash,
                       LinkPairEqual>
        matrix_of_distance_information;
};
