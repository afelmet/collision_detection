...
{
    PrimitiveObject()
        : primitive_type(collision_detection::CYLINDER), radius(0.0), height(0.0), dimensions(3)
    {
    }
...
{
    OctreeDebugConfig()
        : save_octree(false), save_octree_path("./"), save_octree_filename("octree.bt")
    {
    }
 ...
 *  This enum is used to decide whether to calculate only the distance
 * information or multi contact collision information from the collision
 * library.
 */
...
struct CollisionDetectionConfig
{
    CollisionDetectionConfig()
        : collision_library(FCL), collision_info_type(DISTANCE), stop_after_first_collision(true),
          calculate_distance_information(false), max_num_collision_contacts(1)
    {
    }
    ...
    /// Usage: For sampling based planner, it is enough to invalid a state space
    /// if there is a collision in just one link.
    ///        On the other hand if you use optimzation based planners, it is
    ///        better to know all the collisions. As this collision information
    ///        could be used to calculate the collision cost for that iteration.
    ...
    /// @breif number of collision contacts. By default it is set to one. Please
    /// dont set any other value, unless you want that information.
