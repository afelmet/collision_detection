...
// typedef for std::variant of possible collision libraries
// typedef for shared pointer using the variant (do we even need this to be a
// pointer?)
...
class AbstractCollisionDetection
{
  public:
    ...
    virtual void registerOctreeToCollisionManager(const std::shared_ptr<octomap::OcTree> &octomap,
    ...
    virtual void registerOctreeAsBoxesToCollisionManager(
        const std::shared_ptr<octomap::OcTree> &octomap,
        /* const base::Pose &collision_object_pose, */ std::string link_name) = 0;

    virtual bool registerMeshToCollisionManager(const std::string &abs_path_to_mesh_file,
                                                const Eigen::Vector3d &mesh_scale,
                                                const std::string &link_name,
                                                const base::Pose &collision_object_pose,
                                                const double &link_padding) = 0;

    virtual void registerBoxToCollisionManager(const double &box_x, const double &box_y,
                                               const double &box_z, const std::string &link_name,
                                               const base::Pose &collision_object_pose,
                                               const double &link_padding) = 0;

    virtual void registerCylinderToCollisionManager(const double &radius, const double &length,
                                                    const std::string &link_name,
                                                    const base::Pose &collision_object_pose,
                                                    const double &link_padding) = 0;

    virtual void registerSphereToCollisionManager(const double &radius,
                                                  const std::string &link_name,
                                                  const base::Pose &collision_object_pose,
                                                  const double &link_padding) = 0;

    virtual void updateCollisionObjectTransform(std::string link_name,
                                                const base::Pose collision_object_pose) = 0;

    virtual void updateEnvironment(const std::shared_ptr<octomap::OcTree> &octomap,
                                   const std::string &env_object_name) = 0;

    virtual void updateOctomapBoxesEnvironment(const std::shared_ptr<octomap::OcTree> &octomap,
                                               const std::string &env_object_name) = 0;
    ...
    virtual bool removeObjectFromOctree(Eigen::Vector3d object_pose,
                                        Eigen::Vector3d object_size) = 0;

    // The rest of the library expects a 'long unsigned int' instead of an
    // 'int'.
                                 ...
                                 const std::string &second_link_name)
    {
        ...
        // AbstractCollisionDetection::disabled_collisions_.size(); i++ )
        for (const srdf::Model::CollisionPair &dc :
             AbstractCollisionDetection::disabled_collisions_)
        {
    ...
    //         virtual std::vector<DistanceInformation>
    //         &getClosestObstacleToRobotDistanceInfo() = 0;

    //         virtual std::vector<DistanceInformation> &getSelfContacts() = 0;
    //
    //         virtual std::vector<DistanceInformation>
    //         &getEnvironmentalContacts() = 0;
