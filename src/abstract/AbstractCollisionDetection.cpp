...
void AbstractCollisionDetection::setDisabledCollisionPairs(
    std::vector<srdf::Model::CollisionPair> &disabled_collisions)
{
    disabled_collisions_ = disabled_collisions;
}

void AbstractCollisionDetection::addDisabledCollisionPairs(
    srdf::Model::CollisionPair &disabled_collision)
{
...
void AbstractCollisionDetection::removeDisabledCollisionLink(const std::string &link)
{
    ...
    remove_link_ = link;
    disabled_collisions_.erase(
        std::remove_if(disabled_collisions_.begin(), disabled_collisions_.end(),
                       boost::bind(&AbstractCollisionDetection::isLinkListed, this, _1)),
        disabled_collisions_.end());
}

[[nodiscard]] bool
AbstractCollisionDetection::isLinkListed(srdf::Model::CollisionPair const &remove_link)
{
